...
export function useOrder() {
    ...
    const getOrderList = async(id) => {
        const result = await axios.post("http://3.36.70.100:9000/order/all", {"id": id});  //해당 아이디의 주문 정보
    ...
    const saveToOrder = async(orderData) => {
        const result = await axios.post("http://3.36.70.100:9000/order/add", orderData);  
    };
    ...
    const saveGuestOrder = async (guestData, orderData) => {
        try {
            ...
            const guestResponse = await axios.post("http://3.36.70.100:9000/guest/add", guestData);
            ...
            const orderResponse = await axios.post("http://3.36.70.100:9000/guest/addOrder", guestOrderData);
...
//    장바구니에서 선택한 상품 가져오기
    const getCartOrderItems = async (selectedCids) => {
        try {
            const response = await axios.post("http://3.36.70.100:9000/order/cartOrderItems", {
                cids: selectedCids,
            });
    ...
    const saveCartOrders = async (orderDataList) => {
        try {
            await axios.post("http://3.36.70.100:9000/order/addCartOrders", { orders: orderDataList });
        } catch (error) {
    ...
    const deleteOrderedCartItems = async (customerId, orderedItems) => {
        try {
            ...
            const response = await axios.post("http://3.36.70.100:9000/order/deleteOrderedItems", {
